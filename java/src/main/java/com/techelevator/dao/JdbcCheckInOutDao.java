...
@ResponseStatus(HttpStatus.CREATED)
    ...
    public boolean setCheckOutTime(CheckInOut checkOutTime) {
        String sql = "UPDATE user_visits SET check_out_date = ? WHERE user_visit_id = ? ";
        boolean success = false;
        try{
       int numberOfRows = jdbcTemplate.update(sql, checkOutTime.getCheckOutTime(), checkOutTime.getUserVisitId());
            if (numberOfRows == 0) {
